...
typedef struct	s_parentheses_data
{
...
t_list_brackets_pairs		backtracking_possibilities(const std::vector<Token>& tokens, t_parentheses_data data, const unsigned long int pos);
t_list_brackets_pairs		try_pipe_brackets(const std::vector<Token>& tokens, t_parentheses_data data, const unsigned long int pos);
t_list_brackets_pairs		parentheses_brackets(const std::vector<Token>& tokens, t_parentheses_data data, const unsigned long int pos, Token::t_token token_type);
t_bracket					test_external_brackets(t_possibility& p);
bool						handle_operators(t_possibility& possibility, long long int *pos);
void						add_new_possibility(t_list_brackets_pairs& results, const std::vector<Token>& tokens, t_parentheses_data& data, const unsigned long int pos);
